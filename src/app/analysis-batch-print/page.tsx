...
function BatchPrintContent() {
    ...
    useEffect(() => {
        const loadAll = async () => {
            const results: LoadedAssignment[] = [];
            for (let i = 0; i < ids.length; i++) {
                try {
                    const a = await dbService.getAssignmentById(ids[i]);
                    let annotations: Record<string, string> = {};
                    if (studentId) {
                        try {
                            const saved = await dbService.getWordAnnotations(ids[i], studentId);
                    ...
                    if (a) results.push({ assignment: a, annotations });
                } catch (e) {
                    console.error(`Failed to load ${ids[i]}:`, e);
                }
                setProgress(i + 1);
            }
            setLoaded(results);
    ...
    if (loading) {
        return (
            <div className="relative">
                <SkeletonPrintPage />
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg">
                    {progress}/{ids.length} 로드 중...
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    <p className="text-slate-500 text-sm">인쇄할 과제가 없습니다.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="max-w-[780px] mx-auto px-4 py-3 print:p-0 print:max-w-none font-sans text-slate-800" style={{ fontSize: '10px' }}>
            <style jsx global>{`
                @media print {
                    ...
                    /* Large sections should NOT force avoid-break — let them flow across pages */
                    ...
                    /* Ensure each assignment block is visible */
            ...
            {loaded.map(({ assignment, annotations }, assignmentIdx) => {
                ...
                return (
                    <div key={assignment.id || assignmentIdx} className={`print-assignment-block ${assignmentIdx > 0 ? 'page-break' : ''}`}>
                        {/* ═══ HEADER ═══ */}
                        <div className="mb-2 pb-1.5 border-b-2 border-slate-800 avoid-break">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-[8px] text-slate-400 mt-0.5 font-medium tracking-wider">
                                        청한외국어학원 · Deep Analysis
                                        {loaded.length > 1 && <span className="ml-2 text-slate-500">({assignmentIdx + 1}/{loaded.length})</span>}
                                    </p>
                                </div>
...
export default function AnalysisBatchPrintPage() {
    return (
        <Suspense fallback={
            <div className="flex items-center justify-center min-h-screen">
                <SkeletonPrintPage />
