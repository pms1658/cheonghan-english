...
/**
 * Gemini TTS API ?ÑÎ°ù??(?úÎ≤Ñ Ï∫êÏãú ?¨Ìï®)
 * POST /api/tts
...
// ?Ä?Ä Server-side audio cache ?Ä?Ä
const audioCache = new Map<string, { base64: string; timestamp: number }>();
const CACHE_TTL_MS = 2 * 60 * 60 * 1000; // 2?úÍ∞Ñ
...
// ?Ä?Ä WAV header for PCM 16-bit 16kHz mono ?Ä?Ä
...
// ?Ä?Ä Voice name mapping for Google Cloud TTS ?Ä?Ä
...
export async function POST(req: Request) {
    ...
    try {
        ...
        // ?Ä?Ä Call Google Cloud TTS API ?Ä?Ä
            ...
            `https://texttospeech.googleapis.com/v1/text:synthesize?key=${apiKey}`,
            {
                ...
                body: JSON.stringify({
                    ...
                    audioConfig: {
                        ...
                        sampleRateHertz: 16000,          // 16kHz (Studio ?àÏßà ?†Ï?, ?©Îüâ Í∞êÏÜå)
                        speakingRate: 0.95,
                    }
                }),
            }
        ...
        // ?Ä?Ä Wrap raw PCM in WAV header (for browser AudioContext.decodeAudioData) ?Ä?Ä
        const pcmBuffer = Buffer.from(pcmBase64, 'base64');
        const wavHeader = createWavHeader(pcmBuffer.length);
        const wavBuffer = Buffer.concat([wavHeader, pcmBuffer]);
        const wavBase64 = wavBuffer.toString('base64');

        // ?Ä?Ä Cache the WAV ?Ä?Ä
