...
1.  **VERBS (Underline) [word](V)**
    ...
    - **Auxiliary + Adverb + Main Verb** (e.g., "will always love", "can really do", "has never been"):
        ...
        - **?†Ô∏è ZERO DEDUCTION: ALL styles above are EQUALLY valid. You MUST NOT deduct ANY points for choosing one style over another. This is the #1 most common grading error ??do NOT make it.**
...
5.  **SUBORDINATORS (Angle Brackets) < ... >**
    ...
    - **?†Ô∏è PARTICIPIAL CONSTRUCTIONS are NOT subordinate clauses.** They are phrases (conjunction + subject omitted), so they MUST use ( ) modifier brackets:
...
6.  **MODIFIERS (Parentheses) ( ... )**
    - ANY non-backbone element:
        ...
        - **Participial constructions** (Î∂ÑÏÇ¨Íµ¨Î¨∏: -ing/-ed at sentence start/end, reduced from subordinate clause) ??these are PHRASES, not clauses. Always ( ).
...
2.  **Sentence Form Score (10 pts)**: Compare student's selectedForms against correctForms.
    - Use exact string match. Forms: "1?ïÏãù", "2?ïÏãù", "3?ïÏãù", "4?ïÏãù", "5?ïÏãù", "3?ïÏãù ?òÎèô??, "4?ïÏãù ?òÎèô??, "5?ïÏãù ?òÎèô??
    - **?†Ô∏è CRITICAL ??MAIN CLAUSE ONLY**: correctForms must contain the sentence form(s) of the **MAIN CLAUSE(s) (Ï£ºÏ†à) ONLY**. Do NOT include forms for subordinate clauses (Ï¢ÖÏÜç??, relative clauses (Í¥ÄÍ≥ÑÎ?Î™ÖÏÇ¨??Í¥ÄÍ≥ÑÎ??¨Ï†à), adverbial clauses (Î∂Ä?¨Ï†à), or noun clauses (Î™ÖÏÇ¨??. Only independent clauses and coordinate clauses joined by coordinators (and/but/or/so) count.
    - Example: "When he arrived, she gave him the book" ??correctForms: ["4?ïÏãù"] (main clause only; "When he arrived" is an adverbial clause ??IGNORE its form)
    - Example: "He studies hard and she reads books" ??correctForms: ["1?ïÏãù", "3?ïÏãù"] (two coordinate main clauses)
...
- **correctForms** (string[]): One form per MAIN CLAUSE only (Ï£ºÏ†à). Subordinate clauses (Î∂Ä?¨Ï†à, Í¥Ä?Ä?? Î™ÖÏÇ¨?? Ï¢ÖÏÜç?? are excluded. Count only independent/coordinate main clauses.
  - Values: "1?ïÏãù", "2?ïÏãù", "3?ïÏãù", "4?ïÏãù", "5?ïÏãù", "3?ïÏãù ?òÎèô??, "4?ïÏãù ?òÎèô??, "5?ïÏãù ?òÎèô??
...
- **directTranslation** (string): Chunk-by-chunk Korean translation following the original English word order.
  ...
  - **?†Ô∏è NO MARKUP**: Do NOT include any symbols like [qn], [/qn], [V], [??, [O], <, >, /, ( ), etc. in the translation. Output PURE Korean text only. Use / (slash) ONLY as a chunk separator between translated phrases.
    ...
    const gradingPromises = assignments.map(async (task: any, index: number) => {
      ...
      try {
        ...
        try {
          ...
          const safeScore = Math.max(0, Math.min(100, rawScore)); // 0~100 ?¥Îû®??
        ...
        return {
          score: 0,
          feedback: `Ï±ÑÏ†ê Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§. ?§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.`
        };
      }
    });
