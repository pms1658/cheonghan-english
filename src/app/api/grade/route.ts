...
1.  **VERBS (Underline) [word](V)**
    ...
    - **Auxiliary + Adverb + Main Verb** (e.g., "will always love", "can really do", "has never been"):
        ...
        - **? ï¸ ZERO DEDUCTION: ALL styles above are EQUALLY valid. You MUST NOT deduct ANY points for choosing one style over another. This is the #1 most common grading error ??do NOT make it.**
...
5.  **SUBORDINATORS (Angle Brackets) < ... >**
    ...
    - **? ï¸ PARTICIPIAL CONSTRUCTIONS are NOT subordinate clauses.** They are phrases (conjunction + subject omitted), so they MUST use ( ) modifier brackets:
...
6.  **MODIFIERS (Parentheses) ( ... )**
    - ANY non-backbone element:
        ...
        - **Participial constructions** (ë¶„ì‚¬êµ¬ë¬¸: -ing/-ed at sentence start/end, reduced from subordinate clause) ??these are PHRASES, not clauses. Always ( ).
...
2.  **Sentence Form Score (10 pts)**: Compare student's selectedForms against correctForms.
    - Use exact string match. Forms: "1?•ì‹", "2?•ì‹", "3?•ì‹", "4?•ì‹", "5?•ì‹", "3?•ì‹ ?˜ë™??, "4?•ì‹ ?˜ë™??, "5?•ì‹ ?˜ë™??
    - **? ï¸ CRITICAL ??MAIN CLAUSE ONLY**: correctForms must contain the sentence form(s) of the **MAIN CLAUSE(s) (ì£¼ì ˆ) ONLY**. Do NOT include forms for subordinate clauses (ì¢…ì†??, relative clauses (ê´€ê³„ë?ëª…ì‚¬??ê´€ê³„ë??¬ì ˆ), adverbial clauses (ë¶€?¬ì ˆ), or noun clauses (ëª…ì‚¬??. Only independent clauses and coordinate clauses joined by coordinators (and/but/or/so) count.
    - Example: "When he arrived, she gave him the book" ??correctForms: ["4?•ì‹"] (main clause only; "When he arrived" is an adverbial clause ??IGNORE its form)
    - Example: "He studies hard and she reads books" ??correctForms: ["1?•ì‹", "3?•ì‹"] (two coordinate main clauses)
...
- **correctForms** (string[]): One form per MAIN CLAUSE only (ì£¼ì ˆ). Subordinate clauses (ë¶€?¬ì ˆ, ê´€?€?? ëª…ì‚¬?? ì¢…ì†?? are excluded. Count only independent/coordinate main clauses.
  - Values: "1?•ì‹", "2?•ì‹", "3?•ì‹", "4?•ì‹", "5?•ì‹", "3?•ì‹ ?˜ë™??, "4?•ì‹ ?˜ë™??, "5?•ì‹ ?˜ë™??
...
- **directTranslation** (string): Chunk-by-chunk Korean translation following the original English word order.
  ...
  - **? ï¸ NO MARKUP**: Do NOT include any symbols like [qn], [/qn], [V], [??, [O], <, >, /, ( ), etc. in the translation. Output PURE Korean text only. Use / (slash) ONLY as a chunk separator between translated phrases.
      ...
      try {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            ...
            await new Promise(r => setTimeout(r, 1000)); // 1ì´??€ê¸????¬ì‹œ??
          }
        }
        ...
        try {
          ...
          const safeScore = Math.max(0, Math.min(100, rawScore)); // 0~100 ?´ë¨??
        ...
        return {
          score: 0,
          feedback: `ì±„ì  ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤. ?¤ì‹œ ?œë„?´ì£¼?¸ìš”.`
        };
      }
    });
