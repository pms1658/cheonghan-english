        ...
        <div className="flex flex-col h-full bg-slate-50">
            ...
            <div className="px-8 py-4 bg-white border-b border-slate-200 flex justify-between items-center sticky top-0 z-10">
                ...
                <div className="flex gap-2">
                    ...
                    {/* analysis 타입: Step1에서는 지문분석(AI Deep Analysis) 버튼, Step2에서는 저장 완료 */}
                    {assignmentType === 'analysis' ? (
                        step === 1 ? (
                            <button
                                onClick={handleDeepAnalysis}
                                disabled={!formData.content.trim() || isAnalyzing}
                                className="px-4 py-2 text-white font-bold rounded-lg text-sm flex items-center gap-2 transition-all disabled:opacity-50 shadow-sm"
                                style={{ background: isAnalyzing ? '#7c3aed' : 'linear-gradient(135deg, #7c3aed, #6d28d9)' }}
                            >
                                {isAnalyzing ? (
                                    <><svg className="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>분석 중...</>
                                ) : (
                                    <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>지문 분석</>
                                )}
                            </button>
                        ) : (
                            <button
                                onClick={handleCreateFinal}
                                className="px-4 py-2 text-white font-bold rounded-lg text-sm shadow-sm"
            ...
            <div className="flex-1 overflow-y-auto p-8">
                <div className="max-w-4xl mx-auto bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                    {step === 1 ? (
                            ...
                            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
                                    ...
                                    <div className="flex gap-2 mb-2 items-center flex-wrap">
                                        ...
                                        {/* analysis 타입은 AI Deep Analysis가 헤더로 이동 — 여기선 structure 타입에만 표시 */}
                                        {assignmentType === 'structure' && (
                        ...
                        /* Step 2: Sentence review — premium design */
                        <div className="space-y-5">
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-800">문장 분석 확인</h3>
                                        <p className="text-xs text-slate-400">{parsedSentences.length}개 문장 · 저장 전 최종 확인하세요</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 text-xs text-slate-400">
                                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-blue-400 inline-block"/>S</span>
                                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-red-400 inline-block"/>V</span>
                                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-green-400 inline-block"/>O</span>
                                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-orange-400 inline-block"/>C</span>
                                </div>
                            </div>
                                        ...
                                        return (
                                            <div key={idx} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
                                                <div className="flex gap-4 items-start">
                                                    <span className="flex-shrink-0 w-7 h-7 rounded-lg bg-slate-100 text-slate-500 font-bold text-xs flex items-center justify-center">{idx + 1}</span>
                                                    <p className="flex-1 text-slate-800 leading-relaxed font-medium">{sen}</p>
                                                </div>
                                            </div>
                                        );
                                    } else {
                                            ...
                                            <div key={idx} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                                ...
                                                <div className="px-5 py-4 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                                                    <div className="flex gap-3 items-start">
                                                        <span className="flex-shrink-0 w-7 h-7 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 text-white font-bold text-xs flex items-center justify-center shadow-sm">{idx + 1}</span>
                                                        <div className="flex-1 min-w-0">
                                                            {/* 원문 분석 태그 렌더링 */}
                                                            <div className="text-slate-800 font-medium text-base leading-[2.2] mb-2">
                                                                <AnalysisSentenceRenderer text={sen.analyzed || sen.original} />
                                                            </div>
                                                            {/* 해석 */}
                                                            {sen.translation && (
                                                                <div className="flex items-start gap-2">
                                                                    <span className="text-[10px] font-bold text-indigo-400 bg-indigo-50 px-1.5 py-0.5 rounded uppercase tracking-wide flex-shrink-0 mt-0.5">해석</span>
                                                                    <p className="text-slate-500 text-sm leading-relaxed">{sen.translation}</p>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
                                                ...
                                                {/* Grammar + Vocab */}
                                                {((sen.grammar && sen.grammar.length > 0) || (sen.vocab && sen.vocab.length > 0)) && (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                                                        {/* Grammar */}
                                                        {sen.grammar && sen.grammar.length > 0 && (
                                                            <div className="p-4">
                                                                <div className="flex items-center gap-1.5 mb-3">
                                                                    ...
                                                                    <span className="text-[11px] font-bold text-violet-600 uppercase tracking-wider">Grammar Points</span>
                                                                </div>
                                                        ...
                                                        {sen.vocab && sen.vocab.length > 0 && (
                                                            <div className="p-4">
                                                                <div className="flex items-center gap-1.5 mb-3">
                                                                    ...
                                                                    <span className="text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Vocabulary</span>
                                                                </div>
                                                                <div className="flex flex-wrap gap-1.5">
                                                                    {sen.vocab.map((v, i) => (
                                                                        <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-50 border border-emerald-100 rounded-lg text-xs">
                                                ...
                                                {/* Word-by-word (직독직해) if available */}
                                                {(sen as any).wordByWord && (
                                                    <div className="px-4 pb-4">
                                                        <div className="bg-indigo-50 rounded-xl p-3 border border-indigo-100">
                                                            <div className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-1.5">직독직해</div>
                                                            <p className="text-sm text-indigo-700 leading-relaxed">{(sen as any).wordByWord}</p>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
