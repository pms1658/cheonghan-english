...
- ★★★ 대화 자연스러움 규칙 (절대 준수 — 위반 시 문제 난이도 붕괴):
  - **화자는 자신의 조건/선호만 말해야 함** — 구체적으로 어떤 옵션이 탈락되는지 직접 설명 금지!
  - ❌ 금지 예시: "I want blue. So option 3 and 5 are out because they're red."
    → 학생이 표를 안 봐도 답을 알 수 있음
  - ❌ 금지 예시: "Let's not get the $40 one, the $35 one, or the $50 one."
    → 가격을 일일이 나열하며 탈락시키면 표 대조 불필요
  - ✅ 올바른 예시: "I don't want to spend more than $30."
    → 학생이 표에서 $30 초과 옵션을 직접 찾아 제거해야 함
  - ✅ 올바른 예시: "I prefer something made of wood."
    → 학생이 표에서 wood가 아닌 행을 제거해야 함
  - **요약: 조건을 제시하되, "그러면 이것 저것이 빠지네" 같은 보충설명은 절대 넣지 말 것**
  - 대화 상대도 마찬가지: "Oh, then that eliminates..." 같은 정리 발언 금지
  - 자연스러운 대화 = 조건 제시 → 상대방 동의/다음 조건으로 넘어감
...
- **(a)~(e) 배치 규칙 (★★★ 가장 중요 — 43번 정답 유출 방지)**:
  ...
  - **★★★ 성별 대명사 규칙 (절대 준수 — 위반 시 문제 성립 불가)**:
  - (a)~(e) 5개 모두 **반드시 같은 성별** 대명사여야 함
  - 전부 he/him/his 이거나, 전부 she/her/hers — **절대 섞지 말 것!**
  - ❌ 금지 예시: (a)he (b)she (c)he (d)he (e)he → she만 골라내면 바로 답
  - ✅ 올바른 예시: (a)he (b)he (c)he (d)he (e)he → 문맥을 읽어야 누가 다른 인물인지 판별
  - **이를 위해 등장인물을 같은 성별로 설계해야 함** (예: 남자 주인공 + 남자 친구/점원)
  - 4개 = 주인공 / 1개 = 다른 인물 (같은 성별의 조력자, 상대방 등)
  - 정답인 1개: 문맥상 명확히 다른 인물을 가리킴 (성별이 아니라 상황/행동으로 구분)
